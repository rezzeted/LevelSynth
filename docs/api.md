...
- `edgar::geometry::overlap_along_line`, `remove_overlapping_along_lines` — overlap along an orthogonal segment (`polygon_overlap_grid2d`: merged rectangle intervals on the scan line when decomposition exists; Clipper2 fallback if partition is empty). C# `OverlapAlongLine` / `RemoveOverlapping`.
- `edgar::geometry::detail::overlap_along_line_polygon_partition_bruteforce` — brute-force reference for tests (same semantics as production path).

## Graphs

- `edgar::graphs::UndirectedAdjacencyListGraph<T>` — adjacency list.
- `edgar::graphs::is_connected`, `edgar::graphs::is_tree`, `edgar::graphs::get_planar_faces` (planar embeddings via Boost.Graph; `int` vertices only for faces).
...
- `edgar::generator::grid2d::GraphBasedGeneratorGrid2D<TRoom>` — main façade; `set_layout_yield_callback`, `orchestration_stats()`.
- `edgar::generator::grid2d::GraphBasedGeneratorConfiguration`
  ...
  - `layout_stream_mode` — `Single`, `OnEachLayoutGenerated` (yield after successful outer `try_complete_chain`), or `OnEachSaTryCompleteChain` (inner `TryCompleteChain` on a clone after each accepted SA step); `max_layout_yields`.
- `edgar::generator::grid2d::Grid2DLayoutState<TRoom>` — mutable layout state (`clone`, `to_layout_grid()`).
- `edgar::generator::grid2d::LayoutYieldEvent`, `LayoutYieldInfo`, `LayoutOrchestrationStats`, `ChainGenerateContext` — see `layout_orchestration.hpp`.
...
- JSON: `edgar::io::layout_to_json`, `layout_save_json` (see `include/edgar/io/layout_json.hpp`).
