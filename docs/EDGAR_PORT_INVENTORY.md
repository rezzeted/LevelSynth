...
| Генератор | Частично | `GraphBasedGeneratorBackend`: strip **или** chain + SA; CS + constraints; `LayoutControllerGrid2D` (жадность + shape/position SA) подключён к цепному пайплайну; без полного C# `LayoutController` |
...
1. **Полный `LayoutController` (дожим до C#)** — `TryCompleteChain`, отдельный проход коридоров, random restarts, энергия по узлам; базовые CS + жадность + shape/position SA уже в `LayoutControllerGrid2D` / `configuration_spaces_grid2d.*`.
...
- **Generator:** `strip_packing` — детерминированный packer. `chain_simulated_annealing` — цепи + жадность по CS/дверям + fallback jitter + `LayoutControllerGrid2D::evolve` (shape/position + то же SA-расписание, что у `SimulatedAnnealingEvolverGrid2D`). Yield нескольких layout’ов из SA и полный C# `LayoutController` — не портированы.
